...
then
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
    	(
        MONITOR=$mon polybar -q cyn-bar -c "${HOME}"/.config/bspwm/bars/"${DEFAULT_BAR}"/config.ini &
    	MONITOR=$mon polybar -q cyn-bar2 -c "${HOME}"/.config/bspwm/bars/"${DEFAULT_BAR}"/config.ini &
        )
    done
fi
...
then
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		MONITOR=$mon polybar -q marisol -c "${HOME}"/.config/bspwm/bars/"${CURRENT_BAR}"/config.ini &
	done
else
	...
	# This file launch the bar/s
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		MONITOR=$mon polybar -q marisol -c "${HOME}"/.config/bspwm/bars/"${DEFAULT_BAR}"/config.ini &
	done
fi
...
then
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
    	(
        MONITOR=$mon polybar -q cyn-bar -c "${HOME}"/.config/bspwm/rices/"${CURRENT_BAR}"/config.ini &
    	MONITOR=$mon polybar -q cyn-bar2 -c "${HOME}"/.config/bspwm/rices/"${CURRENT_BAR}"/config.ini &
        )
    done
else
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
    	(
        MONITOR=$mon polybar -q cyn-bar -c "${HOME}"/.config/bspwm/rices/"${DEFAULT_BAR}"/config.ini &
    	MONITOR=$mon polybar -q cyn-bar2 -c "${HOME}"/.config/bspwm/rices/"${DEFAULT_BAR}"/config.ini &
        )
    done
fi
...
then
	...
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		MONITOR=$mon polybar -q dani -c "${HOME}"/.config/bspwm/rices/"${CURRENT_BAR}"/config.ini &
	done
else
	...
	# This file launch the bar/s
	for mon in $(polybar --list-monitors | cut -d":" -f1); do
		MONITOR=$mon polybar -q dani -c "${HOME}"/.config/bspwm/rices/"${DEFAULT_BAR}"/config.ini &
	done
fi
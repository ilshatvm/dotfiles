...
then
    ...
    for mon in $(polybar --list-monitors | cut -d":" -f1); do
    	(
        MONITOR=$mon polybar -q mel-bar -c "${HOME}"/.config/bspwm/rices/"${DEFAULT_BAR}"/config.ini &
    	MONITOR=$mon polybar -q mel2-bar -c "${HOME}"/.config/bspwm/rices/"${DEFAULT_BAR}"/config.ini &
        )
    done
fi